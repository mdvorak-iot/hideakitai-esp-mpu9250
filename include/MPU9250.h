...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting &mpu_setting = MPU9250Setting(), i2c_port_t i2c_port = I2C_NUM_0) {
        // addr should be valid for MPU
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7)) {
            ESP_LOGE("mpu9250", "I2C address 0x%2X is not valid for MPU. Please check your I2C address.", addr);
            return false;
        }
        mpu_i2c_addr = addr;
        setting = mpu_setting;
        ...
        if (isConnectedMPU9250()) {
            ...
            if (isConnectedAK8963())
                ...
                ESP_LOGW("mpu9250", "Could not connect to AK8963");
                has_connected = false;
                return false;
            }
        } else {
            ESP_LOGW("mpu9250", "Could not connect to MPU9250");
            has_connected = false;
            return false;
        }
        has_connected = true;
    ...
    bool isConnectedMPU9250() {
        ...
        ESP_LOGD("mpu9250", "MPU9250 WHO AM I = 0x%02X", c);
    ...
    bool isConnectedAK8963() {
        ...
        ESP_LOGD("mpu9250", "AK8963 WHO AM I = 0x%02X", c);
...
private:
    void initMPU9250() {
        ...
        write_byte(mpu_i2c_addr, SMPLRT_DIV,
    ...
    void initAK8963() {
        ...
        write_byte(AK8963_ADDRESS, AK8963_CNTL, (uint8_t) setting.mag_output_bits << 4 |
                                                MAG_MODE);  // Set magnetometer data resolution and sample ODR
        vTaskDelay(10 / portTICK_PERIOD_MS);

        ESP_LOGD("mpu9250", "Mag Factory Calibration Values: %f, %f, %f", mag_bias_factory[0], mag_bias_factory[1], mag_bias_factory[2]);
    }
...
//            }
    ...
    void calibrate_mag_impl() {
        ...
        ESP_LOGI("mpu9250", "Mag Calibration done!");
        ESP_LOGI("mpu9250", "AK8963 mag biases (mG): %f, %f, %f", mag_bias[0], mag_bias[1], mag_bias[2]);
        ESP_LOGI("mpu9250", "AK8963 mag scale (mG): %f, %f, %f", mag_scale[0], mag_scale[1], mag_scale[2]);
    ...
    void collect_mag_data_to(float *m_bias, float *m_scale) {
        ESP_LOGI("mpu9250", "Mag Calibration: Wave device in a figure eight until done!");
        ...
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            ...
            if (MAG_MODE == 0x02) vTaskDelay(135 / portTICK_PERIOD_MS);  // at 8 Hz ODR, new mag data is available every 125 ms
            if (MAG_MODE == 0x06) vTaskDelay(12 / portTICK_PERIOD_MS);   // at 100 Hz ODR, new mag data is available every 10 ms
        }

        ESP_LOGD("mpu9250", "mag x min/max: %d/%d", mag_min[0], mag_max[0]);
        ESP_LOGD("mpu9250", "mag y min/max: %d/%d", mag_min[1], mag_max[1]);
        ESP_LOGD("mpu9250", "mag z min/max: %d/%d", mag_min[2], mag_max[2]);
    ...
    self_test_impl()  // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        for (int ii = 0; ii < 200; ii++) {  // get average current values of gyro and acclerometer
            ...
            read_bytes(mpu_i2c_addr, ACCEL_XOUT_H, 6,
                       &raw_data[0]);          // Read the six raw data registers into data array
            aAvg[0] += (int16_t) (((int16_t) raw_data[0] << 8) |
                                  raw_data[1]);  // Turn the MSB and LSB into a signed 16-bit value
        ...
        ESP_LOGD("mpu9250", "self test: acceleration trim within: %.1f,%.1f,%.1f %% of factory value", self_test_result[0], self_test_result[1], self_test_result[2]);
        ESP_LOGD("mpu9250", "self test: acceleration trim within: %.1f,%.1f,%.1f %% of factory value", self_test_result[0], self_test_result[1], self_test_result[2]);
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress) const {
        ...
        esp_err_t err = i2c_master_cmd_begin(i2c_port_, cmd, 50 / portTICK_PERIOD_MS);
        i2c_cmd_link_delete(cmd);
        if (err != ESP_OK) ESP_LOGE("mpu9250", "i2c read_byte error: %d (%s)", err, esp_err_to_name(err));
    ...
    void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t *dest) const {
        ...
        i2c_master_stop(cmd);
        esp_err_t err = i2c_master_cmd_begin(i2c_port_, cmd, 100 / portTICK_PERIOD_MS);
        i2c_cmd_link_delete(cmd);
        if (err != ESP_OK) ESP_LOGE("mpu9250", "i2c read_bytes error: %d (%s)", err, esp_err_to_name(err));
    }
};
